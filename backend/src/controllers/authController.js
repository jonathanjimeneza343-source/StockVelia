...
export const registrarEmpresaYUsuario = async (req, res) => {
  const { nombre_empresa, nombre_usuario, apellido_usuario, email, password } =
    req.body;
  ...
  try {
    ...
    if (usuarioExiste.rows.length > 0) {
      return res
        .status(400)
        .json({ error: "El correo del usuario ya está registrado." });
    }
...
export const loginUsuario = async (req, res) => {
  ...
  try {
    ...
    await pool.query(
      "UPDATE usuario SET intentos_fallidos = 0, bloqueo_hasta = NULL WHERE id_usuario = $1",
      [usuario.id_usuario],
    );
...
export const solicitarRecuperacion = async (req, res) => {
  ...
  try {
    ...
    await pool.query("DELETE FROM codigos_recuperacion WHERE correo = $1", [
      correo,
    ]);
    ...
    res
      .status(500)
      .json({ error: "Error al enviar el código de recuperación." });
  }
};
...
export const restablecerPassword = async (req, res) => {
  ...
  try {
    ...
    await pool.query(
      "UPDATE usuario SET password = $1, intentos_fallidos = 0, bloqueo_hasta = NULL WHERE correo = $2",
      [passwordEncriptada, correo],
    );
    await pool.query("DELETE FROM codigos_recuperacion WHERE correo = $1", [
      correo,
    ]);
